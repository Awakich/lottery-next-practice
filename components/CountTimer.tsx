...
const CountTimer = () => {
  ...
  const rendered = ({ hours, minutes, seconds, completed }: Props) => {
    if (completed) {
      return (
        <div>
          <h2 className='text-white text-xl text-center'>Ticket Sales CLOSED right now</h2>
        </div>
      )
    }else{
      return(
  ...
  return (
    <div>
      <Countdown date={new Date(expiration * 1000)} renderer={rendered} />
    </div>
  )
}
